    ...
    <>
      ...
      {product.precio_mayor && product.cantidad_minima_mayor && (
        <div className="wholesale-price-info">
          ...
          <strong>S/ {parseFloat(product.precio_mayor).toFixed(2)} c/u</strong>
        </div>
      )}
    </>
  );
...
const ProductCard = ({ product }) => {
  ...
  const discountPercent = product.precio_oferta
    ? Math.round(
        (1 - parseFloat(product.precio_oferta) / parseFloat(product.precio_unitario)) * 100
      )
    : 0;

  // Ya NO retorna <Col> — solo la Card pura
  return (
