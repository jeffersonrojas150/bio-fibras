...
const LoginForm = ({ onAuthChange }) => {
  ...
  // CAMBIO: Obtener la función de login del contexto
  const { login } = useAuth();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // CAMBIO: Llamar a la función de login del contexto
    ...
    <AuthLayout title="Inicia Sesión" subtitle="Accede a tu cuenta">
      <form onSubmit={handleSubmit} className="auth-form">
        ...
        <button type="submit" disabled={isLoading} className="auth-button primary">
          {isLoading ? (<span className="loading-content"><div className="spinner"></div>Iniciando sesión...</span>) : ('Iniciar Sesión')}
        </button>

        <div className="auth-divider"><span>O</span></div>

        <button type="button" onClick={handleGoogleLogin} disabled={isLoading} className="auth-button google">
          <GoogleIcon /> Continuar con Google
        </button>

        <div className="auth-footer">
          <span>¿Aún no tienes cuenta? </span>
          <Link to="/registro" className="auth-link">Crea una cuenta</Link>
        </div>
      </form>
    </AuthLayout>
  );
